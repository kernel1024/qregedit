...
int sam_add_user_to_grp(struct hive *hdesc, int rid, int grp)
{
  ...
  if (!trav_path(hdesc, 0, s, TPF_VK_EXACT)) {
    qf_printf("sam_add_user_to_grp: user # %x not found!\n",rid);
    return(0);
  }
  ...
  if (!sam_get_machine_sid(hdesc, (char *)&msid)) {
    qf_printf("sam_add_user_to_grp: Could not find machine SID\n");
    return(0);
  }
  ...
  if (gverbose) qf_printf("add_user_to_grp: user SID is <%s>\n", str);
  ...
  if (!usrgrplist) {
    qf_printf("sam_add_user_to_grp: user # %x WAS IN NO GROUPS!\n",rid);
  ...
  if (!sarray) {
    qf_printf("sam_add_user_to_grp: group # %x not found!\n",grp);
    FREE(usrgrplist);
    return(0);
  }
  ...
  if (gverbose) qf_printf("usrgrplist-len = %d\n", usrgrplist->len);
  ...
  if (gverbose) {
  qf_printf("add_user_to_grp: grp memberlist AFTER:\n");
